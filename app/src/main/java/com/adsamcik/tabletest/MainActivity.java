...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Table first = new Table(2, false, 16, AppendBehavior.Any);
		first.addData("hello", "world");
		first.addData("hi", "world");
		Table second = new Table(2, true, 16, AppendBehavior.First);
		second.addData("numbered", "world");
		second.addButton("button", view -> {
		});


		Table third = new Table(2, false, 16, AppendBehavior.Any);
		for (int i = 0; i < 10; i++)
		...
		for (int i = 0; i < 10; i++) {
			Table tb = new Table(2, false, 16, AppendBehavior.Any);
